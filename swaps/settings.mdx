...
Every aggregator and indivual source that UniDex aggregates. See each source and its supported chains [below](#Sources).
...
UniDex Instant is a basic multihop aggregator ment to be used on chains without existing aggregators. It checks existing sources for best price and routes between user submitted orders for optimal routing.

Supported Networks: `Neon` `Scroll` `Evmos` `Celo` `Mantle` `Telos`
</ResponseField>

<ResponseField name="Uniswap" type="DEX">
Uniswap native router. Bypasses aggregators and routes directly to Uniswap to support things like memecoins and other tokens that are not supported by aggregators.
